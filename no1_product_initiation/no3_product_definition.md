...
- **多語言設定, Language:** 作為使用者，我想要切換 App 介面語言, 繁中 或 英文，以便使用我熟悉的語言。
...
#### 審閱與決策, Review & Decision
- **常駐式 AI 對話入口, Persistent Entry:** 作為使用者，我想要在底部導航列隨時呼叫 AI 助理，以便下達指令或詢問財務狀況。
- **待確認流程, Pending Review:** 作為使用者，當 OS 通知或 AI 產生帳務時，我想要在首頁看到待確認的項目，而非直接寫入帳本。
- **逐日審閱情境, Day-View Context:** 作為使用者，我想要在審閱待確認項目時，看到它們被嵌入在當日的交易列表中，以便對照上下文 例如避免重複記帳。
- **嵌入式差異檢視, Inline Diff:** 作為使用者，我想要在列表中直接展開查看 "Before vs After" 的欄位變更 如分類從 A 變 B，並能一鍵核准或拒絕。
- **批量操作, Batch Action:** 作為使用者，我想要一次性核准或拒絕當日所有的待確認項目，以節省時間。
...
**定位:** 財務規則運算核心，實現 邏輯除錯器 價值。
**價值:** 自動化價值, Automation, 除錯價值, Debugging

### User Stories

#### 規則管理, Rule Management
- **規則中心, Rule Center:** 作為使用者，我想要一個可視化的介面來查看手動建立或 AI 生成的所有自動化規則。
- **填空式規則編輯, Mad Libs Editor:** 作為手機使用者，我想要透過 當 A 包含 B 時，自動設為 C 的填空介面來快速建立規則，避免複雜的程式編寫。
- **規則開關, Rule Toggle:** 作為使用者，我想要能暫時停用某條規則而不刪除它。

#### 邏輯除錯與模擬, Debug & Simulate
- **時光機回溯, Replay:** 作為使用者，當我修改一條規則後，我想要系統自動模擬這條規則會影響過去哪些交易，並讓我決定是否套用。
- **非破壞性修正, Logic Refactor:** 作為使用者，我想要透過對話 Chat 告訴 AI 我要改變分類邏輯 例如 Steam 都是訂閱類，讓 AI 自動幫我更新規則並重新整理舊帳。
...
- **使用者習慣門檻過高:** 單一使用者養成記帳習慣已屬不易，要求多位使用者同時具備記帳習慣的機率極低。
- **產品複雜度失衡:** 共用帳本功能需要完整的權限管理系統，包含交易可見性控制、成員權限分級等，這會帶來：
   - 產品設計複雜度大幅提升
   - 開發施工成本顯著增加
   - 使用者操作學習曲線陡峭
- **架構衝突難解:** 無論採用 共用單本帳本 或 多本帳本連結 方案，都會面臨難以調和的問題：
   - 共用單本帳本：需要細緻的權限控制以保護隱私
   - 多本帳本連結：會產生資料斷頭與同步一致性問題
- **實際需求場景稀少:** 相對於投入的開發成本，真正需要此功能的使用者比例過低，不符合產品優先級原則。
