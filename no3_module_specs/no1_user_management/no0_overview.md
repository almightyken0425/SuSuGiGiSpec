...
- **User Management 模組負責:**
  - **使用者資料儲存:** 在 Firestore 中維護使用者個人資料
  - **首次登入流程:** 新使用者註冊時建立預設資料
  - **偏好設定管理:** 語言、貨幣、時區等使用者偏好
...
└── no3_apis/
    └── update_preferences_api.md      # 更新使用者偏好設定 API - 選用
```
...
- **首次登入邏輯:**
  - **檢查:** App 端檢查 Firestore 中是否有使用者文件
  - **建立:** 若無，建立預設資料
  - **特性:** 簡單、可靠、冪等

- **偏好設定更新:**
  - **功能:** 使用者可在 App 中修改語言、貨幣、時區
  - **實作:** 透過 Firestore SDK 直接更新
    - 不一定需要 Cloud Function