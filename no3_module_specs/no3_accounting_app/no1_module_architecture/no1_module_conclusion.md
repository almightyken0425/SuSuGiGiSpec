...
- **認證與同步原則**
    ...
    - **流程:**
        - 採納情境式登入 Contextual Login, 僅在需權限操作時要求登入
        - 免費版: 資料僅儲存於本機
...
- **啟動流程原則**
    ...
    - **流程:**
        ...
        - 完成後, 才可執行 批次同步 的自動觸發
- **匯率轉換與報表原則**
    - **核心:** 嚴格直接匯率 Strict Direct Rate
    - **理由:**
        - 避免多重換算 Multi-hop 造成的誤差與不可追溯性
        - 降低遞迴查找效能成本
        - 保持系統單純性 Linear logic
    - **流程:**
        - 系統僅使用「貨幣 -> 基礎貨幣」的直接匯率進行報表計算
        - 若無直接匯率，視為無法計算，不進行中間跳轉
        - UI 不主動提示缺匯率警告，保持介面簡潔
        - 使用者需自行維護由外幣到基礎貨幣的匯率，或透過轉帳自動記錄
        - **注意:** 轉帳自動記錄的匯率若非對應基礎貨幣，因不支援跳轉，則對報表無直接幫助