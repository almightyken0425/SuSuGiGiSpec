...
- **資料處理:**
    ...
    - **總餘額區塊:**
        ...
        - **貨幣邏輯:**
            ...
            - **IF 多幣別:** 主要貨幣換算金額
    - **逐筆紀錄區塊:**
        - **來源:** 本機 DB Transactions 與 Transfers
        - **第一層分組:** 固定分為支出與收入兩個區塊
        ...
        - **交易過濾:**
            - **條件:** `當前時間區間` + `所選帳戶`
        - **轉帳過濾:**
            - **條件:** `當前時間區間` + 轉帳涉及所選帳戶與非所選帳戶
            - **規則:** 若所選帳戶為 A, B
                - 顯示 A 或 B 對外部帳戶的轉帳
                - A 與 B 互轉的紀錄不顯示
            - **整合至支出收入:**
                - **IF 所選帳戶為轉出帳戶:** 視為支出，列於支出區
                - **IF 所選帳戶為轉入帳戶:** 視為收入，列於收入區
            - **分組依據:**
                - **類別分組模式:** 以外部帳戶作為分組
                    - 支出區：A/B → C 歸為「帳戶 C」分組，A/B → D 歸為「帳戶 D」分組
                    - 收入區：C → A/B 歸為「帳戶 C」分組，D → A/B 歸為「帳戶 D」分組
                - **日期分組模式:** 依交易日期分組
        - **分組金額計算:**
        ...
        - **單筆紀錄:**
            ...
            - **轉帳紀錄金額:**
                - **顯示原則:** 以所選帳戶視角顯示
                - **轉出:** 使用 `amountFrom`（所選帳戶轉出的金額）
                - **轉入:** 使用 `amountTo`（所選帳戶收到的金額）
                - **IF 單幣別:** 顯示該金額
                - **IF 多幣別:** 顯示該金額 + 主要貨幣換算金額
        - **排序:**
            - **分組間:** 依總金額降冪
...
- **互動:**
    ...
    - **時間區間切換:**
        ...
        - **限制:**
            - 禁止向未來日期區間滑動
            - **IF 無任何交易或轉帳紀錄:** 僅顯示當前時間區間，禁止向過去滑動
            - **IF 有交易或轉帳紀錄:** 可向過去滑動至最早一筆紀錄所在的時間區間
