...
- **絕對禁止順序性描述:**
    - **嚴禁**使用 `第一`, `第二`, `第三` 等順序數字。
    - **嚴禁**使用 `階段一`, `階段二`, `階段三` 等階段描述。
    - **嚴禁**使用 `下一步`, `再下一步`, `最後`, `推薦起點` 等順序性文字。
    - **原因:** 順序性描述會導致中間插入項目時需要重新編號或修改文字。
    - **替代:** 使用描述性標題，順序由視覺排版位置暗示。
    - **範例** (拒絕):
        ```markdown
        ### 第一階段:核心功能
        ### 第二階段:進階功能
    ...
    - **範例** (推薦):
        ...
        ```
- **絕對禁止計數數字:**
    - **嚴禁**在摘要中使用 `識別出 X 個`, `共 X 個`, `總計 X 個` 等計數描述。
    - **原因:** 計數會導致新增或刪除項目時需要手動更新數字。
    - **替代:** 清單本身已說明有多少項目，無需額外標註。
    - **範例** (拒絕):
        ```markdown
        識別出 11 個核心模組:
        - 模組 A
