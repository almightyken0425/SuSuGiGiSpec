...
- **資料處理:**
    - **圓餅圖區塊:**
        ...
        - **過濾:**
          - `當前時間區間` + `所選帳戶`
        - **貨幣邏輯:**
          - 依 CurrencyConversionLogic 進行換算
        - **顏色邏輯:**
            - **依據:**
              - `圖表來源` 設定，支出或收入
            - **邏輯:**
              - 僅有當前 `圖表來源` 的前 5 大類別使用彩色圖表色，其餘顯示預設色
        - **排序:**
          - 依總額降冪排列
            - **合併條件:**
                - 總區塊數 <= 6
                ...
                - 獨立顯示類別數 <= 5
            - **提前合併:**
              - 若前 N 類別累計佔比 >= 5/6 且 N < 5，則 N+1 及其後併入其他
    - **總餘額區塊:**
        - **過濾:**
          - `當前時間區間` + `所選帳戶`
        - **貨幣邏輯:**
    ...
    - **逐筆紀錄區塊:**
        - **來源:**
          - 本機 DB Transactions 與 Transfers
        - **來源:**
          - 本機 DB Transactions 與 Transfers
        - **第一層分組:**
          - 固定分為支出與收入兩個區塊，順序依 `圖表來源` 決定，若選支出則支出在前，若選收入則收入在前
        - **第二層分組:**
          - 依 Filter Modal 選擇
            - 類別分組
            - 日期分組
        - **交易過濾:**
          - `當前時間區間` + `所選帳戶`
        - **轉帳過濾與顯示:**
          - 依 CurrencyConversionLogic 的轉帳紀錄顯示規則
        - **分組依據:**
            - **類別分組模式:**
              - 以外部帳戶作為分組
            - **日期分組模式:**
...
- **Footer 新增按鈕導航:**
    ...
    - **流程:**
        - **權限驗證:**
            - **驗證:**
              - 讀取 `PremiumLogic.checkPremiumStatus()`
            - **IF True:**
              - 執行導航
            - **IF False:**
              - 執行限制檢查
            - **查詢本機 DB:**
                - `active_accounts` = `Accounts.Count`
                    - **條件:**
                      - deletedOn IS null AND disabledOn IS null
