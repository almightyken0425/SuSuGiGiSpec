...
- **設定列表:**
    ...
    - **本地化:**
        - `主要貨幣`
            ...
            - **邏輯:**
                - **更新設定:** 更新 `baseCurrencyId`
                - **主動建立預設匯率:**
                    - **動作:** 掃描所有既有外幣帳戶
                    - **檢查:** 針對每個外幣，檢查對 新本幣 是否有匯率紀錄
                    - **IF 無紀錄:** 寫入一筆 `Rate=1`, `RateDate=Now` 的 `CurrencyRates`
                - **不自動換算 No Auto-Triangulation:**
                    - 系統 **嚴禁** 使用交叉匯率 Triangulation 自動計算新匯率 例如: 不可拿 JPY / OldBase 除以 USD / OldBase 來算出 JPY / USD。
                    - **理由:** 避免累計匯差誤差，確保報表精確度。
                    - **行為:** 承上，無直接紀錄者一律建立 Rate=1 的新紀錄，不試圖推算。
        ...
        - `時區`
            ...
            - **預設值:** 初始化時自動偵測手機系統時區
            - **影響:** 變更後，App 內所有日期顯示、報表計算邊界皆應以此時區為準
        - `語系`
            - **顯示:** 當前 App 顯示的語言
            - **互動:** 點擊開啟支援的語言列表
